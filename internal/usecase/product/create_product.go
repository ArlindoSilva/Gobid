...
import (
	...
	"github.com/ArlindoSilva/gobid/internal/validator"
)
...
const minActionDuration = 2 * time.Hour
...
func (req CreateProductReq) Valid(ctx context.Context) validator.Evaluator {
	...
	eval.CheckField(req.Baseprice > 0, "baseprice", "this field must be greater than zero")

	eval.CheckField(req.AuctionEnd.Sub(time.Now()) >= minActionDuration, "auction_end", "must be at least two hours duration")
